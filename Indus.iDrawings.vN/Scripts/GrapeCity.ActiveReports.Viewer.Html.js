...
!function(window) {
    ...
    function ArReportService(options, resourceManager) {
        ...
        function open(reportId, parameters) {
            var strReport = "";
            var reportDataJSON = JSON.parse(reportId);
            reportDataJSON["CustomReportFields"]= "";
            reportDataJSON["CustomReportFilterQuery"] = "";
            strReport = JSON.stringify(reportDataJSON);
            g_reportPathforExport = strReport
            var Data = [];
            for (i = 0; i < reportDataJSON["ListReportFieldIdValues"].length; i++) {
                var obj = reportDataJSON["ListReportFieldIdValues"][i];
                if (obj["ReportFieldId"] == 271 || obj["ReportFieldId"] == 346 || obj["ReportFieldId"] == 147 || obj["ReportFieldId"] == 3428) {
                    Data.push(obj);
                }
            }
            reportDataJSON["ListReportFieldIdValues"] = JSON.parse(JSON.stringify(Data));
            strReport = JSON.stringify(reportDataJSON);
            g_reportPath = strReport;
        ...
        function exportImpl(doctoken, exportType, settings) {
            function translateSettings(ss) {
                var newSettings = {};
                newSettings.reportPath = g_reportPath;
                newSettings.__RequestVerificationToken = $('input[name="__RequestVerificationToken"]').val();
    ...
    function ArsReportService(options, resourceManager) {
        ...
        function exportImpl(token, exportType, settings) {
            ...
            return $base.exportDocument(description, token.exportType(exportType), exportOptions).then(function(url) {
                return isAbsoluteUri(url) || (url = resourceHandler + url, settings && settings.saveAsDialog && (url += "&Attachment=1")), 
                url;
            });
        }
    ...
    function ArsWcfReportService(service, serviceUrl, options, resourceManager, loadResource) {
        ...
        var defaultPollingTimeout = 1e3, pollingTimeout = options.pollingTimeout || defaultPollingTimeout, reportTypes = {
            ...
            Cancelled: 4,
            Rejected: 5
        }, SR = resourceManager && $.isFunction(resourceManager.get) ? resourceManager.get : identity, $base = ReportServiceBase(service, serviceUrl);
        return {
            getReport: getReport,
            resolveParameters: resolveParameters,
    ...
    function BrowserSpecific() {
        ...
        return {
            HaveBuiltInPdfViewer: isEdge() || isFirefox(),
            SupportPdfFrame: isChrome() || isSafari(),
            SupportAnchorDownloadAttribute: isEdge() || isFirefox() || isChrome() || isOpera(),
    ...
    function DocumentModel(reportService, token, reportType) {
        ...
        function load() {
            token = token + "&reportPath=" + g_reportPath;
            token = token + "&__RequestVerificationToken=" + $('input[name="__RequestVerificationToken"]').val();
        ...
        var _toc = ko.observable(nullToc), _reportType = reportType, _actionItemsResolver = new ActionItemsResolver(), _search = reportService && reportService.search && reportService.search.apply ? function(searchOptions) {
    ...
    function PrintingService(resourceManager, browserSpecific) {
        ...
        function openPopup(exportUrl) {
            var printWindow = null;
            try {
                printWindow = window.open("about:blank", "_blank"), printWindow && printWindow.focus();
            } catch (e) {}
            if (printWindow) {
                var html = [];
                html.push("<html>"), html.push("<head><title>" + getResourceString("printing.title") + "</title></head>"), 
                html.push('<body style="height:100%; width:100%; margin:0; padding:0">'), html.push('<object data="' + encodeURI(exportUrl) + '" type="application/pdf" width="100%" height="100%" style="display:block; overflow:hidden; margin:0; padding:0">'), 
                html.push("<p>" + getResourceString("printing.alt").format('<a href="' + encodeURI(exportUrl) + '">' + getResourceString("printing.downloadPDF") + "</a>") + "</p>"), 
                html.push("</object>"), html.push("</body>"), html.push("</html>");
                var markup = printWindow.document;
                markup.open(), markup.write(html.join("")), markup.close();
            }
            return !!printWindow;
        }
        function useFrame(exportUrl) {
            var printingFrameId = "gc-viewer-print-frame", iframe = $("#" + printingFrameId);
            ...
            iframe.css({
                ...
                padding: "0",
                borderWidth: "0"
            }), $("body").append(iframe)) : iframe.attr("src", ""), iframe.attr("src", encodeURI(exportUrl));
        }
        function useAnchor(exportUrl) {
            var link = document.createElement("a");
            link.setAttribute("href", encodeURI(exportUrl)), link.setAttribute("download", "Report.pdf"), 
        ...
        function printImpl(documentModel) {
            return documentModel["export"](ExportType.Pdf, {
                ...
                return exportUrl ? browserSpecific.HaveBuiltInPdfViewer ? void (!openPdf(exportUrl) && browserSpecific.SupportAnchorDownloadAttribute && documentModel["export"](ExportType.Pdf, {
                    printing: !0,
                    saveAsDialog: !0
                }).done(function(exportUrl2) {
                    exportUrl2 && useAnchor(exportUrl2);
                })) : browserSpecific.SupportPdfFrame ? void useFrame(exportUrl) : browserSpecific.SupportAnchorDownloadAttribute ? void useAnchor(exportUrl) : void (openPopup(exportUrl) || useFrame(exportUrl)) : void 0;
            });
        }
        var getResourceString = resourceManager && resourceManager.get || identity;
        return {
    ...
    function ReportServiceBase(service, serviceUrl, setSecurityToken) {
        ...
        function setAuthToken(ajaxOptions) {
            var secruityTokenAdded = setSecurityToken && setSecurityToken(ajaxOptions);
            return secruityTokenAdded, ajaxOptions;
        }
        ...
        function postJson(url, data, skipError, async) {
            ...
            $.ajax(setAuthToken({
                ...
                headers: { 'gel': gel(JSON.stringify(data)), "__RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val() },
                async: async
            }));
        }
        function get(url) {
    ...
    function ReportServiceSelector(options, resourceManager) {
        ...
        if (!options.url) throw new Error("options has no valid url");
        var impl, api = {};
        return [ "open", "close", "run", "validateParameters", "validateParameter", "getPageCount", "getPage", "getToc", "export", "search", "drillthrough", "toggle", "ping", "getState", "setState" ].forEach(function(method) {
    ...
    function ResourceManager() {
        ...
        embedded = {
            ...
            "printing.alt": "This browser does not support PDFs. Please download the PDF to print it: {0}.",
            "printing.downloadPDF": "Download PDF",
            "error.CannotExport": "Cannot export. Please turn off popup blocker.",
    ...
    function ToolbarViewModel(services, viewer) {
        ...
        var getResourceString = services.resourceManager && services.resourceManager.get || identity, _pageNumber = ko.computed({
            read: function() {
                    ...
                    exec: function() {
                        viewer.document()["export"](exportType, {
                            ...
                            if (exportUrl) {
                                ...
                                try {
                                    exportWindow = window.open(exportUrl, "_blank"), exportWindow && exportWindow.focus();
                                } catch (e) {}
                                if (!exportWindow && services.browserSpecific.SupportAnchorDownloadAttribute) {
                                    var link = document.createElement("a");
                                    link.setAttribute("href", encodeURI(exportUrl)), link.setAttribute("download", "Report"), 
                                    document.body.appendChild(link), link.click(), document.body.removeChild(link);
                                } else exportWindow || viewer.handleError(getResourceString("error.CannotExport"));
                            }
    ...
    function ViewerModel(reportService, options) {
        ...
        var nullDocument = DocumentModel(), _reloadingOnInteraction = !1, _pageIndex = ko.observable(0), _location = ko.observable({
            ...
            set report(value) {
                viewer.clearReports(!0), viewer.clearErrors(), setReport(value), value.state() === ReportModelState.noReport && value.open(_manualRun);
            },
            get document() {
                return _document;
            },
...
function exportReportToPDF() {
    var report = new Object({
        reportData: g_reportPathforExport
    });

    var data = new Object({
        reportDataInJson: report,
        type: 2
    });
    var reportData = JSON.parse(g_reportPathforExport);
    var filename = reportData.ExportFileName + ".pdf";
    var postData = { "jsonData": data };
    var gel = gelExport(JSON.stringify(postData));
    $.ajax({
        url: "ReportExport/CommonExport", type: 'POST', headers: { "__RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val(), 'gel': gel }, data: JSON.stringify(postData), success: function (resultData) {
            ...
            if (resultData._body == "Data is Null") {
                ...
                var contentType = "octet/stream";
                var linkElement = document.createElement('a');

                try {
                    var data = base64ToArrayBuffer(JSON.parse(JSON.stringify(resultData).trim()));
                    var blob = new Blob([data], { type: contentType });

                    if ((/Edge/.test(navigator.userAgent)) || (/msie|Trident/.test(navigator.userAgent))) {
                        window.navigator.msSaveOrOpenBlob(blob, filename);
                    } else if (/Safari/.test(navigator.userAgent) && /Apple Computer/.test(navigator.vendor)) {
                        window.open('data:attachment/csv;charset=utf-8,' + encodeURI(blob));
...
function exportReportToExcel() {
    ...
    var reportData = JSON.parse(g_reportPathforExport);
    var filename = reportData.ExportFileName + ".xls";
    var postData = { "jsonData": data };
    var gel = gelExport(JSON.stringify(postData));
    $.ajax({
        url: "ReportExport/CommonExport", type: 'POST', headers: { "__RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val(), 'gel': gel }, data: JSON.stringify(postData), success: function (resultData) {
...
function exportReportToRTF() {
    ...
    var reportData = JSON.parse(g_reportPathforExport);
    var filename = reportData.ExportFileName + ".doc";
    var postData = { "jsonData": data };
    var gel = gelExport(JSON.stringify(postData));
    $.ajax({
        url: "ReportExport/CommonExport", type: 'POST', headers: { "__RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val(), 'gel': gel }, data: JSON.stringify(postData), success: function (resultData) {
...
function exportReportToText() {
       if (GrapeCity.ActiveReports.onTextExportClick != undefined) {
        GrapeCity.ActiveReports.onTextExportClick();
       }
}

function zoomIn() {
   
    if (GrapeCity.ActiveReports.onZoomInClick != undefined) {
        GrapeCity.ActiveReports.onZoomInClick();
    }
...
function zoomOut() {
    ...
    if (GrapeCity.ActiveReports.onZoomOutClick != undefined) {
        GrapeCity.ActiveReports.onZoomOutClick();
    }
}
