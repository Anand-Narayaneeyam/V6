...
xBrowser.prototype._registerEntityCallBacks = function (element, entity) {
    ...
    */
    element.addEventListener('mouseDown', function (e) {
        self._fire('entityMouseDown', { entity: entity, event: e, element: element });
        e.stopPropagation();
    });
    /**
    ...
    */
    element.addEventListener('mouseUp', function (e) {
        self._fire('entityMouseUp', { entity: entity, event: e, element: element });
        e.stopPropagation();
    });
    /**
    ...
    */
    element.addEventListener('mouseMove', function (e) {
        self._fire('entityMouseMove', { entity: entity, event: e, element: element });
        e.stopPropagation();
    });
    /**
    ...
    */
    element.addEventListener('touch', function (e) {
        self._fire('entityTouch', { entity: entity, event: e, element: element });
        e.stopPropagation();
    });
};
...
xBrowser.prototype._renderTreeView = function (container, roots, initSimpleTree, entityTemplate) {
    ...
    searchField.id = container.id;
    searchField.placeholder = "Search..";
    searchField.onkeyup = function () { searchValueChange(searchField); }
    container.appendChild(searchField);

    var treeContainerDiv = document.createElement('div');
    treeContainerDiv.id = "treeContainer";
    container.appendChild(treeContainerDiv);
    ...
    var renderEntities = function (entities, ul) {
        for (var i = 0; i < entities.length; i++) {
            ...
            if (!ul) {
                ...
                treeContainerDiv.appendChild(ul);
            }
            ...
            if (jQuery && jQuery.ui) {
                ...
                if (container.id == 'structure') {
                    $(li).prepend('<input type="checkbox" checked style="float: left;" id=' + entity.id + ' onchange=checkBoxChange(id)>');
                }
    ...
    //        var entity = browser._model.getEntity(this.id);
    //        if ($(this).is(":checked")) {
    //            ShowHideItem(entity, true);
    //        }
    //        else {
    //           ShowHideItem(entity, false);
    //        }
    //    }
    //});
...
var timeout = null;
var searchResult = [];
function searchValueChange(textBox)
{
    //console.log(textBox.value);
    var searchViewId = textBox.parentElement.id;
    var that = this;
    if (timeout !== null) {
        clearTimeout(timeout);
    }
    timeout = setTimeout(function () {
        SearchFunc(textBox.value, searchViewId);
    }, 100);
  
}

function SearchFunc(searchText,parentId) {
    
    if (searchText == "") {
        jQuery("#" + parentId + " ul > li").each(function () {
           
            jQuery(this).find('ul').hide();
            jQuery(this).find('ul span').removeClass('ui-icon-triangle-1-s');
            jQuery(this).find('ul span').addClass('ui-icon-triangle-1-e');

        });
        jQuery("#" + parentId + " ul > li").show();
        jQuery("#" + parentId + " ul span").removeClass('ui-icon-triangle-1-s');
        jQuery("#" + parentId + " ul span").addClass('ui-icon-triangle-1-e');
        
    }
    else {
        jQuery("#" + parentId + " ul > li").each(function () {

            if (jQuery(this).text().search(new RegExp(searchText, "i")) < 0 && !jQuery(this).hasClass('show')) {
                jQuery(this).hide();
                jQuery(this).find('ul span').removeClass('ui-icon-triangle-1-s');
                jQuery(this).find('ul span').addClass('ui-icon-triangle-1-e');
                ...
                jQuery(this).show();
                //jQuery(this).removeClass('ui-icon-triangle-1-e');
                //jQuery(this).addClass('ui-icon-triangle-1-s');
                // jQuery(this).find('ul > li').show();
                jQuery(this).find('ul').show();
                jQuery(this).find('ul span').removeClass('ui-icon-triangle-1-e');
                jQuery(this).find('ul span').addClass('ui-icon-triangle-1-s');
...
//function searchInData(array, searchText, divId)
//{
//    //for (var i in this._model.facility) {

//    //    var node = this._model.facility[i];
...
//            var temp;
//            if (o.displayName.includes(searchText)) {
//                return true;
//            }
...
function checkBoxChange(id) {
    if (id != undefined) {
        var entity = browser._model.getEntity(id);
        if ($('#' + id).is(":checked")) {
            ShowHideItem(entity, true);
        }
        else {
...
*/
xBrowser.prototype.load = function (source, Ids) {
    ...
    if (source instanceof Blob || source instanceof File) {
        ...
        fReader.onloadend = function () {
            if (fReader.result) {
                ...
                self._model = utils.getVisualModel(data,Ids);
