...
function InitViewer(byteArray, jsonData, productIds, productlabel)
{
    //viewer set up
        ...
        viewer.on("mouseMove", function (args) {
            if (args.id != null) {
                ...
                if (entity != null) {
                    ...
                    $("#quickProperties").css({ position: "absolute", right: "auto", bottom: "auto", top: args.clientY + 30, left: args.clientX + 30, opacity: 1 });
...
function OpenFile(modelId, customer, productlabel) {
    try {
        ...
        $.ajax({
            url: 'api/Data/GetWexBimData',
            type: "POST",
            dataType: "json",
            data: JSON.stringify([modelId, customer, productlabel]),
            ...
            success: function (returnObject) {
                if (returnObject.returnCode == 0) {
                  ...
                  //  InitBrowser(productlabel);
                    ...
                    InitViewer(WexBimData, jsonData, productIds + parentIds, productlabel);
...
function OpenJsonFile(modelId, customer, productlabel) {

    try {
       // $('#modelIndicator').show();
        $.ajax({
            url: 'api/Data/GetJsonData',
            type: "POST",
            dataType: "json",
            data: JSON.stringify([modelId, customer, productlabel]),
            contentType: 'application/json; charset=utf-8',
...
window.onload = function () {
    if ((filename != "" && customer != "") || (filename != "" && productlabel != "")) {
        ...
        OpenFile(filename, customer, productlabel);// loading model from iDrawings
