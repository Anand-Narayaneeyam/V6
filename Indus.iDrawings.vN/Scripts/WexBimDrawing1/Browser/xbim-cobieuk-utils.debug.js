...
xCobieUkUtils.prototype.getVisualEntity = function (entity, type) {
    ...
    return new xVisualEntity({
        ...
        displayName: this.getValidationStatus(entity) + entity.Name
    });
};

function getVisualExternalEntityType(entity, Id) {
    if (!entity || !Id) throw 'entity must be defined';
    return new xVisualEntity({
        id: Id,

        name: entity, //prepend validation status. This will make it easier for later.
        description: entity,
        // attributes: this.getAttributes(entity),
        // properties: this.getProperties(entity),
        // assignments: this.getAssignments(entity, type),
        // documents: this.getDocuments(entity, type),
        // issues: this.getIssues(entity),
        displayName: entity.replace("Ifc","")
    });
}
...
xCobieUkUtils.prototype.getVisualModel = function (data,Ids) {
    ...
    var facility = this.getSpatialStructure(data, types, IdArray);
...
xCobieUkUtils.prototype.getSpatialStructure = function (data, types, IdArray) {
    ...
    for (var i in floors) {
        var site = floors[i];
        if (site.ExternalEntity == "IfcSite") {
            vSite = this.getVisualEntity(site, 'site');
            ////////added to avoid undefined in tree////////////
            if (vSite.displayName == "undefined") {
                vSite.displayName = "Site";
            }
            ////////added to avoid undefined in tree////////////
            if (IdArray.length != 0) {
                if (tempArray.indexOf(vSite.id) != -1) {
                    facility.children.push(vSite);
                    tempArray.splice(tempArray.indexOf(vSite.id), 1);
                }
            }
            else {
                facility.children.push(vSite);
    ...
    for (var i in floors) {
        var building = floors[i];
        if (building.ExternalEntity == "IfcBuilding") {
            vBuilding = this.getVisualEntity(building, 'building');
            ////////added to avoid undefined in tree////////////
            if (vBuilding.displayName == "undefined") {
                vBuilding.displayName = "Building";
            }
            ////////added to avoid undefined in tree////////////
            vBuilding.id = vBuilding.id + '_1'; //added bcoz of building and project had same Ids.
            if (IdArray.length != 0) {

                if (tempArray.indexOf(vBuilding.id) != -1) {
                    if (vSite)
                        vSite.children.push(vBuilding);
                    else
                        facility.children.push(vBuilding);
                    tempArray.splice(tempArray.indexOf(vBuilding.id), 1);
                }
            }
            else {
                if (vSite)
    ...
    for (var i in floors) {
        ...
        if (IdArray.length != 0)
        {
            if (tempArray.indexOf(vFloor.id) != -1)
            {
                if (vBuilding)
                    vBuilding.children.push(vFloor);
                else
                    facility.children.push(vFloor);
                tempArray.splice(tempArray.indexOf(vFloor.id), 1);
            }
        }
        else
        {
        ...
        var spaces = floor.Spaces.filter(function (e) {return e.ExternalEntity =="IfcSpace" });
        if (!spaces) continue;
        var spaceCategory = [];
        ...
        for (var s in spaces) {
            ...
            var externalEntity = getVisualExternalEntityType(space.ExternalEntity,space.ExternalId + "_1");
            //spaceCategory.indexOf(externalEntity) === -1 ? spaceCategory.push(externalEntity) : console.log("This item already exists");

            var spc = spaceCategory.filter(function (e) { return e.name == space.ExternalEntity });
            if (spc.length == 0)
            {
                spaceCategory.push(externalEntity);
            }

            var vSpace = this.getVisualEntity(space, 'space');
            if (vSpace.description)
            { vSpace.description
            vSpace.displayName = vSpace.description.concat('-', vSpace.displayName)
            }
            if (IdArray.length != 0) {
                if (tempArray.indexOf(vSpace.id) != -1)
                {              
                    vFloor.children.push(vSpace);
                    tempArray.splice(tempArray.indexOf(vSpace.id), 1);
                }
            }
            else
            {
                var vSpaceCategory = spaceCategory.filter(function (e) { return e.name == space.ExternalEntity })[0];
                vSpaceCategory.children.push(vSpace);
            }
              //  vFloor.children.push(vSpace);
        }
        for (var i in spaceCategory)
        {
            vFloor.children.push(spaceCategory[i]);
        }
    ...
    for (var t in types) {
        ...
        for (var i in type.children) {
            ...
            var tempExternalEntity = instance.properties.find(function (e) { return e.id == "ExternalEntity" });
            var tempExternalEntityId = instance.properties.find(function (e) { return e.id == "ExternalId" });
            var externalEntity = '';
            var externalEntityId = '';
            if (tempExternalEntity && tempExternalEntityId) {
                externalEntity = tempExternalEntity.value;
                externalEntityId = tempExternalEntityId.value;
            }

            var floorObj = vBuilding.children.find(function (e) { return e.name == spaceName });
            if (floorObj)
            {
                var externalEntityObject = floorObj.children.find(function (e) { return e.name == externalEntity });
                if (!externalEntityObject) {

                    externalEntityObject = getVisualExternalEntityType(externalEntity, externalEntityId + '_1');
                    floorObj.children.push(externalEntityObject);
                }
                externalEntityObject.children.push(instance);
            }
            else
                {
            for (var j = 0; j < vBuilding.children.length; j++) {

                

                var floor = vBuilding.children[j];
                for (var i in floor.children) {

                    var spaceCategory = floor.children[i];
                    var space = spaceCategory.children.filter(function (e) { return e.name == spaceName })[0];

                    // var space = floor.children.filter(function (e) { return e.name == spaceName })[0];
                    if (!space) continue;

                    if (IdArray.length != 0) {
                        if (tempArray.indexOf(instance.id) != -1) {
                            space.children.push(instance);
                            tempArray.splice(tempArray.indexOf(instance.id), 1);
                        }
                    }
                    else
                    {
                        var externalEntityObject = space.children.find(function (e) { return e.name == externalEntity });
                        if (!externalEntityObject)
                        {
                       
                            externalEntityObject = getVisualExternalEntityType(externalEntity, externalEntityId + '_1');
                            space.children.push(externalEntityObject);
                        }
                        externalEntityObject.children.push(instance);
