...
import {Component, OnInit, Output, SimpleChange, OnChanges, DoCheck, KeyValueDiffers, EventEmitter, ChangeDetectorRef, ViewEncapsulation } from '@angular/core';
...
@Component({
    ...
    providers: [AdministrationService, ObjectsService, NotificationService, ConfirmationService, GeneralFunctions, ExportToExcel],
    inputs: [],
    encapsulation: ViewEncapsulation.None,
    directives: [GridComponent, PagingComponent, SubMenu, PageComponent, Notification, ConfirmationComponent, SlideComponent, FieldComponent, SplitViewComponent, ConfigureComponent]

})
export class ImportDataComponent {
    imprtCategoryfield: IField[];
    importControlfield: IField[];
    importGridFieldObject=[];
    fieldObject: IField[];
    strSheetName: any;
    ...
    addtnlcategoryId = 0;
    excelData: any[];
    importDataFromExcel: any[];
    fileData: any;
    excelColumnNames: any;
    classId = 0;
    importcategoryId: any;
    templateId: any;
    mappingtable: any;
    importColumnsList: any;
    showField = false;
    showExcel = false;
    splitviewInput: ISplitView = { showSecondaryView: false, showButton: false, secondaryArea: 70 };
    splitViewTarget = false;
    splitViewGridTarget = false;
    itemsSource: any[];
    inputItems: IGrid = { dataKey: "Id", allowAdd: false, allowEdit: false, sortDir: '', sortCol: '' };
    enableMenu = [1];
    splitViewTitle = "";
    drawingCategoryValue = "-1";
    slideMsg = "";
    attributeStatus = false;
    classIdValue = "-1";
    className = "";
    assetimportFields: any;
    showSlide = false;
    Position = "top-right";
    width = 300;
    disabled = false;
    ...
    constructor(private administrationService: AdministrationService, private objectsService: ObjectsService, private _notificationService: NotificationService, private confirmationService: ConfirmationService, private generFun: GeneralFunctions, private exportObject: ExportToExcel) {
    ...
    reloadConfigMapping(initial?: any)
    {
        var contextObj = this;
        if (initial != true)
        {
        ...
        contextObj.administrationService.GetSavedImportColumns(contextObj.excelColumnNames, contextObj.importcategoryId, 0).subscribe(function (resultData1) {
            var excelMappingTableData = new Array<excelMappingTable>();
    ...
    createFieldControls(contextObj, id) {
        debugger
        contextObj.showField = true;
        if (contextObj.importcategoryId == 2) {
            contextObj.splitViewTitle = "Map Space Details"
    ...
    assetValidation() {
        ...
        if (errorMsg1.length == 0) {
            var contextObj = this;
            var importFields = new Array<ImportObjectDetailsInput>();
            var relationId = 0;
            if (contextObj.importControlfield[5].FieldValue == "32") {
                relationId = 1;
            }
    ...
    onMenuClick(value)
    {
        ...
        contextObj.exportObject.ExportData(JSON.stringify(contextObj.itemsSource), contextObj.importGridFieldObject, "Import Data", function (retCode) {
            ...
            else { contextObj._notificationService.ShowToaster("Only Data matches the search criteria can be exported", 3); }
        });
    }
...
export interface ImportObjectDetailsInput {
    Assigned: number;
    AutoNumbering: number;
    ObjectClassName: string;
    DwgCategoryId: number;
