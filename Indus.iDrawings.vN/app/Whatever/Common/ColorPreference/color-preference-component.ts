import { Component, Output, EventEmitter, AfterViewInit, Input, OnChanges, ElementRef, SimpleChanges } from '@angular/core';
...
import { HatchPatternComponent } from '../../../Framework/Whatever/DynamicControls/DynamicFields/hatchpattern.component';
...
@Component({
    ...
    directives: [SlideComponent, LabelComponent, ColorPickerComponent, HatchPatternComponent, Notification],
    providers: [CommonService, HTTP_PROVIDERS, NotificationService, GeneralFunctions, SpaceService],
    inputs: ['moduleId', 'objectCategoryId']
})

export class ColorPreferenceComponent implements AfterViewInit {
    target = 1;
    moduleId: string;
    ...
    itemSource: any;
    DataKey;
    isnotfocus: boolean = true;
    isUpadte: boolean = true;
    isrepetition: boolean = true;
    iscolorchange: boolean = true;
    oldColorPickerValue: any = "";
    ...
    output: any;
    isMySettings: boolean;
    isSharedSpaceColor: boolean = false;
    ...
    @Input() Previlages;
    el: any;
    objectModules = [6, 7, 8, 17, 18, 27, 24, 25, 26];
    constructor(private commonService: CommonService, private notificationService: NotificationService, private generFun: GeneralFunctions, private spaceService: SpaceService, private elemRef: ElementRef) {
        this.el = elemRef;
    }
    ...
    onChangeInput(e, value, fieldName) {
        ...
        tempArray = tempArray.filter(function (el) {
            if (el.Color != null) {
                ...
                this.arrColorArray.find((colorevent) => {
    ...
    update(event) {
        ...
        if (context.moduleId == "3" || context.moduleId == "5") { //space or Employee module
    ...
    getMultipleFieldValuesAsReportFieldArray(fieldObject: any, DataKey) {
        ...
        //    });
        //}
        if (Number(this.moduleId) == 3) { //for space module
    ...
    onChangePattern(e, value, fieldName) {
