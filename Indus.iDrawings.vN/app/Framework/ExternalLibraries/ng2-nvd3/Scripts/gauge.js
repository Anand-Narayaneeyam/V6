    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            g_needle.append('circle')
                .attr('cx', 0)
                .attr('cy', 0)
                .attr('r', 0.115 * (radius-100));  //changes for idrawings radius -> radius - 100
            ...
            var valueText;
            var readingCount = 0;
            for (var j = 10; j < max; j = j + 20) {
                // readingCount = readingCount + 20;
                 var ReadingsXY = valueToPoint(j, 0.8, j, j+20, radius)
               // valueText = "";
                valueText = gEnter.append("text")
                                    .attr('id', "Value")
                                    .attr("font-size", fontSize * 0.45 + "px")
                                    .attr("text-anchor", "middle")
                                    .attr("dy", ".5em")
                                    .attr('x', ReadingsXY.x) //ReadingsXY.x
                                    .attr('y', ReadingsXY.y) //ReadingsXY.y
                                    .style("fill", '#000000');
                valueText.text("120")
                .attr('transform', 'translate(' + (cx + radius / 1.25 - fontSize * 0.9) + ',' + (cy + radius / 1.35 - fontSize * 0.9) + ')');
               //.attr("transform", function () { return "translate(" + cx + ", " + (cy - 0.08 * radius) + ") rotate(270)" });
            ...
            function buildNeedlePath(value, range, cx, cy, min, max, radius) {
                ...
                var head1 = centerPoint(valueToPoint(value - delta, 0.06, min, max, radius), cx, cy); //changes for idrawings 0.12 -> 0.06
                var head2 = centerPoint(valueToPoint(value + delta, 0.06, min, max, radius), cx, cy); //changes for idrawings 0.12 -> 0.06

                var tail = centerPoint(valueToPoint(tailValue, 0, min, max, radius), cx, cy);
                var tail1 = centerPoint(valueToPoint(tailValue - delta, 0.06, min, max, radius), cx, cy); //changes for idrawings 0.12 -> 0.06
                var tail2 = centerPoint(valueToPoint(tailValue + delta, 0.06, min, max, radius), cx, cy); //changes for idrawings 0.12 -> 0.06
