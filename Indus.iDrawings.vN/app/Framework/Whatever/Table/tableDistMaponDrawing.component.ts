import { Component, Input, forwardRef, EventEmitter, Inject, OnChanges, SimpleChanges, Output} from '@angular/core';
import { Notification} from '../../../Framework/Whatever/Notification/notify.component';
import { NotificationService } from '../../../Framework/Models/Notification/notify.service';
import {AppComponent } from '../../../app.component';
import {IField} from  '../../Models/Interface/IField';
import { LabelComponent } from '../../Whatever/DynamicControls/DynamicFields//labelcomponent.component';
import { ColorPickerComponent } from '../../Whatever/DynamicControls/DynamicFields/colorpickercomponent.component';
...
@Component({
    ...
    directives: [Notification, LabelComponent, ColorPickerComponent, CustomCheckBoxComponent],
    providers: [NotificationService, GeneralFunctions, AdministrationService],
    inputs: ['fields', 'fieldData', 'disableButton', 'buttonName'],
})

export class TableDistMaponDrawingComponent implements OnChanges{
    @Input() fieldObject;
    @Input() itemSource;
    @Input() DataKey;
    ...
    DatakeyValues: any;
    updateFieldObjects: any;
    itmSource: any[];
    private fieldObjectRef: any;
    ...
    constructor(private genaralFun: GeneralFunctions, private administrationService: AdministrationService) {

    };

    widthOfHeader(value: any) {
    ...
    updateField(fldObj: IField, Value: any): IField {
        fldObj.FieldValue = Value[fldObj.FieldLabel.trim()];
        fldObj.IsMandatory = false;
        return fldObj;
    }

    onChangeInput(e, value, fieldName) {
        debugger
    ...
    update() {
        ...
        for (var i = 0; i < this.updateFieldObjects.length; i++) {
            var check1 = <HTMLInputElement>document.getElementById("tableDistMap_"+this.itemSource[i].Id);
            if (check1.checked == true) {
                for (var j = 0; j < this.fieldObject.length; j++) {
                    if (j != 1)
                        newField[j].FieldValue = this.updateFieldObjects[i][this.fieldObject[j].FieldLabel];
                    else {
                        var check = <HTMLInputElement>document.getElementById("tableDistMap_" +this.itemSource[i].Id);
    ...
    selectOptions(event) {
        ...
        var check = <HTMLInputElement>document.getElementById("SelectAll");
