...
var labelcomponent_component_1 = require('../../Whatever/DynamicControls/DynamicFields//labelcomponent.component');
var colorpickercomponent_component_1 = require('../../Whatever/DynamicControls/DynamicFields/colorpickercomponent.component');
...
var TableDistMaponDrawingComponent = (function () {
    function TableDistMaponDrawingComponent(genaralFun, administrationService) {
        this.genaralFun = genaralFun;
        this.administrationService = administrationService;
        this.Updatetable = new core_1.EventEmitter();
        this.disableButton = true;
        this.arrColorArray = new Array();
        this.isAllChecked = true;
        this.isChecked = true;
    }
    TableDistMaponDrawingComponent.prototype.ngOnInit = function () {
        this.fieldObject;
        this.DatakeyValues = [];
        this.height = (window.innerHeight - 230).toString() + "px";
        this.fieldObjectRef = JSON.parse(JSON.stringify(this.fieldObject));
        this.disableButton = this.disableButton;
    };
    TableDistMaponDrawingComponent.prototype.ngOnChanges = function (changes) {
    ...
    ;
    TableDistMaponDrawingComponent.prototype.widthOfHeader = function (value) {
        if (value.IsEnabled == true)
            return 200;
        else
            return 0;
    };
    TableDistMaponDrawingComponent.prototype.displayOfHeader = function (value) {
        if (value.IsEnabled == true)
            return "";
        else
            return "none";
    };
    TableDistMaponDrawingComponent.prototype.updateField = function (fldObj, Value) {
        fldObj.FieldValue = Value[fldObj.FieldLabel.trim()];
        fldObj.IsMandatory = false;
        return fldObj;
    };
    TableDistMaponDrawingComponent.prototype.onChangeInput = function (e, value, fieldName) {
        debugger;
        ...
        for (var i = 0; i < this.itemSource.length; i++) {
            if (this.itemSource[i][this.DataKey] == Id) {
                this.itemSource[i][fieldName] = (e.AutoCadColorId);
            }
        }
        this.itemSource = this.itemSource;
    };
    TableDistMaponDrawingComponent.prototype.update = function () {
        var context = this;
        ...
        this.updateFieldObjects = this.itemSource;
        var newField = this.fieldObjectRef;
        var fieldObjectArray = [];
        for (var i = 0; i < this.updateFieldObjects.length; i++) {
            var check1 = document.getElementById("tableDistMap_" + this.itemSource[i].Id);
            if (check1.checked == true) {
                for (var j = 0; j < this.fieldObject.length; j++) {
                    if (j != 1)
                        newField[j].FieldValue = this.updateFieldObjects[i][this.fieldObject[j].FieldLabel];
                    else {
                        var check = document.getElementById("tableDistMap_" + this.itemSource[i].Id);
                        if (check.checked == true) {
                            newField[j].FieldValue = this.updateFieldObjects[i][this.fieldObject[0].FieldLabel];
                        }
                        else {
                            newField[j].FieldValue = null;
                        }
                    }
                }
        ...
        this.Updatetable.emit({
            ReportFieldIdValues: this.output, checkcount: fieldObjectArray.length
        });
    };
    TableDistMaponDrawingComponent.prototype.getMultipleFieldValuesAsReportFieldArray = function (fieldObject, DataKey) {
        var outputList = new Array();
        var entityList = new Array();
        var arrayList = new Array();
        var entityValue = 0;
        var objRef = JSON.parse(JSON.stringify(fieldObject));
        if (objRef.length > 0) {
            for (var i = 0; i < objRef.length; i++) {
                for (var j = 0; j < objRef[i].length; j++) {
                    var fldObject = objRef[i][j];
                    if (fldObject.FieldLabel == DataKey) {
                        entityValue = fldObject.FieldValue;
                    }
                    else {
                        if ((fldObject.IsComputationalField == null || fldObject.IsComputationalField == undefined) || fldObject.IsComputationalField == false) {
                            if (fldObject.FieldValue == undefined) {
                                fldObject.FieldValue = null;
                            }
                            if (fldObject.ReportFieldId == 765) {
                                fldObject.FieldValue = 1;
                            }
                            if (fldObject.ReportFieldId == 766) {
                                fldObject.FieldValue = 0;
                            }
                            if (fldObject.ReportFieldId == 767) {
                                fldObject.FieldValue = 15;
                            }
                            arrayList.push({
    ...
    TableDistMaponDrawingComponent.prototype.getColorDataArray = function () {
        var contextObj = this;
    ...
    TableDistMaponDrawingComponent.prototype.selectAllOptions = function (event) {
        for (var i = 0; i < this.itemSource.length; i++) {
    ...
    TableDistMaponDrawingComponent.prototype.selectOptions = function (event) {
        var count = 0;
        for (var i = 0; i < this.itemSource.length; i++) {
            var check = document.getElementById("tableDistMap_" + this.itemSource[i].Id);
            if (check.checked == true) {
        ...
        if (count == this.itemSource.length) {
            this.isAllChecked = true;
            check.checked = true;
        }
        else {
    ...
    ], TableDistMaponDrawingComponent.prototype, "fieldObject", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], TableDistMaponDrawingComponent.prototype, "itemSource", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], TableDistMaponDrawingComponent.prototype, "DataKey", void 0);
    __decorate([
        core_1.Output(), 
        __metadata('design:type', Object)
    ], TableDistMaponDrawingComponent.prototype, "Updatetable", void 0);
    TableDistMaponDrawingComponent = __decorate([
        core_1.Component({
            selector: 'tableDistMaponDrawing',
            templateUrl: 'app/Framework/Views/Table/tableDistMaponDrawing.component.html',
            styleUrls: ['app/Framework/Views/Table/table.css'],
            directives: [notify_component_1.Notification, labelcomponent_component_1.LabelComponent, colorpickercomponent_component_1.ColorPickerComponent, checkboxcomponent_component_1.CustomCheckBoxComponent],
            providers: [notify_service_1.NotificationService, General_1.GeneralFunctions, administration_service_1.AdministrationService],
            inputs: ['fields', 'fieldData', 'disableButton', 'buttonName'],
        }), 
        __metadata('design:paramtypes', [General_1.GeneralFunctions, administration_service_1.AdministrationService])
    ], TableDistMaponDrawingComponent);
    return TableDistMaponDrawingComponent;
}());
exports.TableDistMaponDrawingComponent = TableDistMaponDrawingComponent;
//# sourceMappingURL=tableDistMaponDrawing.component.js.map