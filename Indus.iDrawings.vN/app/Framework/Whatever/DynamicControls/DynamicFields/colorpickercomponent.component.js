...
var ColorPickerComponent = (function () {
    function ColorPickerComponent(administrationService) {
        this.administrationService = administrationService;
        this.displayStyle = "none";
        this.hiddenDiv = "block";
        this.setBackgroundColor = "#FF0000";
        this.colorArray0 = new Array();
        this.colorArray1 = new Array();
        this.colorArray2 = new Array();
        this.colorArray3 = new Array();
        this.colorArray4 = new Array();
        this.colorArray5 = new Array();
        this.colorArray6 = new Array();
        this.colorArray7 = new Array();
        this.colorArray8 = new Array();
        this.colorArray9 = new Array();
        this.arrColorArray = new Array();
        this.blnShowIcon = true;
        this.blnPickerExpanded = false;
        this.colorPickerValueChange = new core_1.EventEmitter();
    }
    ColorPickerComponent.prototype.ngOnInit = function () {
        ...
        this.colorChange();
        var contextObj = this;
        contextObj.itemSource = contextObj.getColorDataArray();
        for (var i = 0; i < contextObj.itemSource.length; i++) {
            if (i < 25) {
                contextObj.colorArray0.push({
                    AccessColorId: contextObj.itemSource[i].AccessColorId,
                    AutocadColorId: contextObj.itemSource[i].AutocadColorId,
                    ColorId: contextObj.itemSource[i].ColorId,
                    RGB: contextObj.itemSource[i].RGB,
                    BackGroundColor: "#" + contextObj.itemSource[i].RGB
                });
            }
            if (i > 24 && i < 50) {
                contextObj.colorArray1.push({
                    AccessColorId: contextObj.itemSource[i].AccessColorId,
                    AutocadColorId: contextObj.itemSource[i].AutocadColorId,
                    ColorId: contextObj.itemSource[i].ColorId,
                    RGB: contextObj.itemSource[i].RGB,
    ...
    ColorPickerComponent.prototype.colorChange = function () {
        var contextObj = this;
        if (this.fieldObject) {
            if (this.fieldObject.FieldValue == "" || (this.fieldObject.FieldValue == null)) {
                this.blnShowIcon = true;
            }
            else {
                var autocadColorId;
                ...
                arrColorData = contextObj.getColorDataArray();
                this.blnShowIcon = false;
                var status = false;
                if (arrColorData != undefined) {
                    autocadColorId = this.fieldObject.FieldValue;
                    for (var i = 0; i < arrColorData.length; i++) {
                        if (arrColorData[i].AutocadColorId == autocadColorId) {
                            status = true;
                            rgbColor = arrColorData[i].RGB;
                        }
                        if (status == false) {
                            rgbColor = "FF0000";
                        }
                    ...
                    rgbColor = "#" + this.fieldObject.FieldValue;
                }
    ...
    ColorPickerComponent.prototype.ColorPickerToggle = function () {
    ...
    ColorPickerComponent.prototype.KeyPressColorPickerToggle = function (Keyevent) {
    ...
    ColorPickerComponent.prototype.KeyPresscolorOnClick = function (Keyevent, autocadColorId, rgb, accessColorId) {
        var key = Keyevent.keyCode || Keyevent.which;
        if (key == 13 || key == 32) {
            Keyevent.preventDefault();
            this.colorOnClick(autocadColorId, rgb, accessColorId);
        }
        else if (key == 9 && !Keyevent.shiftKey) {
            if (this.colorArray7[(this.colorArray7.length - 1)].AutocadColorId == autocadColorId) {
                var colorpicker = document.getElementById("ColorPickerDiv");
                if (this.displayStyle == "block") {
                    this.displayStyle = "none";
                    this.hiddenDiv = "none";
                }
    ...
    ColorPickerComponent.prototype.colorOnClick = function (autocadColorId, rgb, accessColorId) {
        var rptFieldId = this.fieldObject.ReportFieldId;
        var selectedValue = this.fieldObject.FieldValue;
        this.ColorPickerToggle();
        this.blnShowIcon = false;
        this.setBackgroundColor = rgb;
        this.fieldObject.FieldValue = autocadColorId;
        this.colorPickerValueChange.emit({
            AutoCadColorId: autocadColorId,
            RGBValue: rgb,
            AccessColorId: accessColorId,
            SelectedFieldValue: selectedValue,
            ReportFieldId: rptFieldId
        });
    };
    ColorPickerComponent.prototype.getColorDataArray = function () {
        var contextObj = this;
    ...
    ], ColorPickerComponent.prototype, "colorPickerValueChange", void 0);
    ColorPickerComponent = __decorate([
        core_1.Component({
            selector: 'ColorPicker',
            templateUrl: 'app/Framework/Views/DynamicControls/DynamicFields/colorpicker.component.html',
            inputs: ['fieldObject', 'showLabel'],
...
//# sourceMappingURL=colorpickercomponent.component.js.map